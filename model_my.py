...
class DiTBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, drop_path=0., attn_drop=0., qkv_bias=True):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size)
        self.attn = nn.MultiheadAttention(hidden_size, num_heads, batch_first=True, bias=qkv_bias)
...
class DiTBlock2D(nn.Module):
    def __init__(self, dim, num_heads):
        super().__init__()
        self.block = DiTBlock(dim, num_heads)
    ...
    @staticmethod
    def patches2img(x, h, w):
        # x: (B, N, C) -> (B, C, H, W)
        return x.transpose(1, 2).reshape(-1, x.size(-1), h, w)

    def forward(self, x):
        b, c, h, w = x.shape
        x_flat = self. img2patches(x)
        x_flat = self.block(x_flat)
        x = self.patches2img(x_flat, h, w)
        return x
...
class MultiscaleNet(nn.Module):
    def __init__(self,
        ...
        self.encoder_level1_small = nn.Sequential(*[
            DiTBlock2D(dim=dim, num_heads=heads[0]) for i in range(num_blocks[0])])

        self.down1_2_small = Downsample(dim)
        self.encoder_level2_small = nn.Sequential(*[
            DiTBlock2D(dim=int(dim * 2 ** 1), num_heads=heads[1]) for i in range(num_blocks[1])])

        self.down2_3_small = Downsample(int(dim * 2 ** 1))
        self.latent_small = nn.Sequential(*[
            DiTBlock2D(dim=int(dim * 2 ** 2), num_heads=heads[2]) for i in range(num_blocks[2])])
        ...
        self.decoder_level1_small = nn.Sequential(*[
            DiTBlock2D(dim=int(dim * 1 ** 1), num_heads=heads[0]) for i in range(num_blocks[0])])
        ...
        self.latent_norm = nn.InstanceNorm2d(4, affine=True)  # 如果 latent 是4通道，其他通道数就改成相应数字

    def forward(self, inp_img):
        outputs = list()
        vae_out = self.vae.encode(inp_img)
        inp_img = vae_out.latent_dist.sample()
        inp_img = self.latent_norm(inp_img)
        inp_img_max = inp_img
        ...
        for output in outputs[::-1]:  # 已经反转的输出列表
            # 拆开 decode 和 sample
            decoded_dist = self.vae.decode(output)
            decoded = decoded_dist.sample
